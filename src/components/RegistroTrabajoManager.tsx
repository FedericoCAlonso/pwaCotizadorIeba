...
import { HardHat, Plus, Save, Trash2, TrendingUp, X } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-on-surface-variant mt-1">Horas ejecutadas en obra para calibrar las Tareas Tipo.</p>
        </div>
      ...
      <div className="bg-surface-container-low border border-outline-variant/20 rounded-3xl p-5 space-y-4 shadow-sm">
        <h3 className="text-xs font-semibold text-on-surface uppercase tracking-wider flex items-center gap-2">
          <TrendingUp className="w-4 h-4 text-primary" />Calibración: Estimado vs. Real
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {taskVariance.map((tv) => (
            <div key={tv.tarea.id} className="bg-surface-container-highest/50 p-4 rounded-2xl border border-outline-variant/20 space-y-2">
              ...
              <div className="text-xs text-on-surface-variant flex justify-between"><span>Estimado:</span><span className="font-mono text-primary">{tv.horasEstimadasUnidad} hs/{tv.tarea.unidad}</span></div>
              <div className="text-xs text-on-surface-variant flex justify-between"><span>Real ({tv.nMuestras} reg.):</span><span className="font-mono text-primary font-bold">{tv.nMuestras > 0 ? `${formatNumber(tv.horasRealesPromedioUnidad)} hs/${tv.tarea.unidad}` : '—'}</span></div>
              {tv.nMuestras >= 3 && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-on-surface">
            ...
            <tbody className="divide-y divide-outline-variant/20">
              {registros.length === 0 ? (
                <tr><td colSpan={7} className="px-5 py-12 text-center text-on-surface-variant text-sm font-medium">Sin registros cargados.</td></tr>
              ) : (
                registros.map((r) => {
